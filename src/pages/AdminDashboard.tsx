        ...
        <Sidebar variant="inset" collapsible="icon" className="bg-white">
                ...
                <div className="space-y-2 px-2">
                  {activeAlerts.map((alert) => (
                    <div 
                      key={alert.id}
                      className={cn(
                        "p-2 text-xs rounded-md cursor-pointer hover:bg-gray-100 transition-colors",
                        alert.severity === 'high' && "border-l-2 border-red-500",
                        alert.severity === 'medium' && "border-l-2 border-amber-500",
                        alert.severity === 'low' && "border-l-2 border-blue-500"
                      )}
                      onClick={() => handleAlertAction(alert.id, 'view')}
                    >
                      <div className="font-medium flex items-center gap-1">
                        {alert.type === 'sos' && <AlertTriangle className="h-3 w-3 text-red-500" />}
                        {alert.type === 'delay' && <Clock className="h-3 w-3 text-amber-500" />}
                        {alert.type === 'maintenance' && <Bell className="h-3 w-3 text-blue-500" />}
                        Bus #{alert.bus}
                      </div>
                      <div className="text-gray-600 mt-1">{alert.message}</div>
                      <div className="text-gray-400 mt-1 text-[10px]">{alert.time}</div>
                    </div>
                  ))}
                </div>
              </SidebarGroupContent>
          ...
          <SidebarFooter>
            <div className="px-2 py-2">
              <Button variant="outline" size="sm" className="w-full justify-start" onClick={() => toast.success("Logged out successfully")}>
                <span>Logout</span>
              </Button>
            </div>
          </SidebarFooter>
